...
import { Package, Plus, Search, Trash2, ImageIcon, Info, Loader2, AlertCircle } from 'lucide-react';
import { formatCurrency } from '../utils';

const Items: React.FC = () => {
  const { items, addItem, deleteItem, loading } = useAppStore();
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button 
          ...
          className="bg-[#001A33] text-white px-4 py-2.5 rounded-xl font-bold hover:bg-slate-800 transition-all flex items-center gap-2"
        >
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border shadow-sm p-6 space-y-6 animate-in fade-in slide-in-from-top-4 duration-300">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex flex-col gap-1.5">
              ...
              <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="p-3 border rounded-xl bg-slate-50 text-sm outline-none" />
            </div>
          ...
          <button type="submit" className="w-full bg-[#EAB308] text-[#001A33] font-black py-4 rounded-xl shadow-xl hover:opacity-90">SALVAR NO CATÁLOGO</button>
        </form>
      ) : (
        <div className="space-y-4">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={20} />
            <input type="text" placeholder="Buscar..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-3 rounded-xl border bg-white outline-none shadow-sm" />
          </div>
...
const ItemRow = ({ item, onDelete }: any) => {
